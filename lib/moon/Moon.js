...
export const lunationNumber = (date = new Date()) => {
  return (
    Math.round(
      (JulianDay.fromDate(date) - LUNATION_BASE_JULIAN_DAY) / SYNODIC_MONTH
    ) + 1
  );
};
...
export const lunarDistance = (date = new Date()) => {
  ...
  const percent =
    2 * Math.PI * normalize((julian - 2451562.2) / ANOMALISTIC_MONTH);

  return (
    60.4 -
    3.3 * Math.cos(percent) -
    0.6 * Math.cos(2 * radians - percent) -
    0.5 * Math.cos(2 * radians)
  );
};
...
export const nextLunarPhase = (phase, date = new Date()) => {
  if (!isDate(date)) {
    throw new Error(`Expected '${date}' to be a date`);
  }
